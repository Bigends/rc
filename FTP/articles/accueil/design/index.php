...
<div class="center">
    ...
    if (isset($_GET['action'])) {
        if ($_GET['action'] == 'add_article') {
            ...
            if (isset($_POST['submit'])) {
                ...
                $save_article = $db->prepare('INSERT INTO design(titre,contenu,images_name,plus) VALUES(?,?,?,?)');
    ...
    <?php
            ...
            require_once("includes/database.php");
            $req_all_article = $db->prepare('SELECT *FROM design');
        ...
        <?php
            ...
            $delete_article = $db->prepare('DELETE FROM design WHERE id=?');
            ...
            if (isset($_POST['submit'])) { // Mis à jour
                ...
                $article_update = $db->prepare('UPDATE design set titre=?,plus=?,contenu=? WHERE id=?');
            ...
            $req_all_value = $db->prepare('SELECT *FROM design WHERE id=?');
